...
2. Hit **💾 Save context** → the engine summarizes it into a HandoffNote and
   extracts durable facts; the middle **Vault** panel fills live.
...
4. **🔍 Search memory** for a decision — semantic match, not keyword.
...
│       ├── save/route.ts   # POST — engine.save (summarize + facts + index)
...
└── lib/
    ...
    ├── chat.ts           # Claude chat + --no-ai demo reply
...
- **`--no-ai` / demo fallback** — no key for the configured provider ⇒ chat returns
  a clearly labelled canned reply and save stores raw text. Never a dead button. Set a
  **~$5 budget cap** on the key you use.
...
Add `apps/playground/.env.local` with a `CTXVAULT_MODEL` and the matching key (real
HandoffNotes + facts), and optionally `CTXVAULT_EMBED_MODEL` + its key (real
embeddings). See `.env.example` for every supported provider. Without them it runs in
demo mode — still fully clickable.

The AI/embeddings badges in the header report the model actually in use, so a
...
- **Environment variables (all optional):** `CTXVAULT_MODEL`, `CTXVAULT_EMBED_MODEL`,
  and the key for whichever provider you picked — `ANTHROPIC_API_KEY`,
...
1. The main barrel pulls in `better-sqlite3` (a native binary) and the OKF file
   ...
   `/web` exports only the portable pieces (engine + MemoryAdapter + LLM + embedders).
...
4. Byte-for-byte the same `CtxEngine` class, summarizer, embedder, retriever, and
   fact logic run in both. Only the injected `StorageAdapter` differs
